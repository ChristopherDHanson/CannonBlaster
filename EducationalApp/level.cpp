...
void Level::createDynamicBall(float32 radius, float32 posX, float32 posY, b2Vec2 force)
{
    if (textures.size() == 0) {
        throw std::runtime_error("Textures array empty");
    }
    ...
    b2Body* body = world->CreateBody(&bodyDef);
    b2CircleShape ball;
    ball.m_radius = radius/2.0f;
    //box.SetAsBox(width/2.0f, height/2.0f);
    body->CreateFixture(&ball, 1.0f);
    ...
    b2Vec2 temp = bodies[bodies.size() - 1]->GetPosition();
    sprite->setOrigin(radius/2.0f,radius/2.0f);
    sprite->setPosition(temp.x, -1 * temp.y);
    sprites.push_back(sprite);
}

void Level::createDynamicBox(float32 width, float32 height, float32 posX, float32 posY, b2Vec2 force)
{
    if (textures.size() == 0) {
        throw std::runtime_error("Textures array empty");
    }
